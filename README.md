...
<img width="1200" height="900" alt="pca_pooled_degs" src="https://github.com/user-attachments/assets/d15b8ff7-5df4-44ee-ae46-e50dfc9dab43" />
<img width="1200" height="900" alt="volcano_pooled" src="https://github.com/user-attachments/assets/4128d3f9-dc22-454c-a3b0-0860e4f56223" />
<!-- Add 1-2 key figures here, e.g. the DEG-restricted PCA (Figure 3) and the chamber-stratified volcano plots (Figure 6) -->
<!-- ![PCA on significant DEGs](plots/pca_degs.png) -->
...
```
Run a single-sample test with `bash rnaseq_pipeline_refined.sh --test`, or the full 10-sample run with `bash rnaseq_pipeline_refined.sh`.
...
All three apply apeglm shrinkage to log2 fold changes and classify DEGs at padj < 0.05, |LFC| > 1. See the rendered [`DESeq2_pipeline_refined.html`](DESeq2_pipeline_refined.html) for full output with all figures inline.
...
- **Trimming:** Trimmomatic (paired-end; LEADING:3, TRAILING:3, MINLEN:36) retained **93.7–99.9% of read pairs** across samples — see [pre-trim](pre_trim_multiqc_report.html) and [post-trim](post_trim_multiqc_report.html) MultiQC reports.
...
Full table: [`Sample_Table.xlsx`](Sample_Table.xlsx)


## Repository Structure
```
├── rnaseq_pipeline_refined.sh      # Bash preprocessing: SRA → aligned counts
├── DESeq2_pipeline_refined.Rmd     # R analysis: 3 DESeq2 models + all figures
├── DESeq2_pipeline_refined.html    # Rendered analysis notebook
├── Sample_Table.xlsx               # Donor/chamber/condition/GEO sample map
├── pre_trim_multiqc_report.html    # QC before trimming
├── post_trim_multiqc_report.html   # QC after trimming
├── report/
│   ├── Assignment1_Preprocessing_Report.docx   # Preprocessing/QC write-up
│   └── Final_Team_Report.docx                  # Full analysis write-up
├── presentation/
